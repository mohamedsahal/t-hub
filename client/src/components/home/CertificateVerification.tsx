...
const certificateSchema = z.object({
  certificateId: z.string().min(5, "Certificate ID is required").regex(/^THB-\d{4}-\d{5}$/, "Invalid certificate format (e.g., THB-2023-12345)")
});
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: data.message || "Failed to verify certificate. Please try again.",
          variant: "destructive",
        });
      }
    } catch (error) {
    ...
    <section className="py-16 bg-primary-dark text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto text-center mb-12">
          <h2 className="text-3xl font-bold font-inter">Verify Your Certificate</h2>
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-gray-700">Certificate ID</FormLabel>
                        <FormControl>
                          <Input
